...
func loadShipRoutes(s ship.Router, routes []*Route) {
	for _, r := range routes {
		switch r.Method {
		case "GET":
			s.Get(r.Path, shipHandler(r.Method, r.Path))
		case "POST":
			s.Post(r.Path, shipHandler(r.Method, r.Path))
		case "PATCH":
			s.Patch(r.Path, shipHandler(r.Method, r.Path))
		case "PUT":
			s.Put(r.Path, shipHandler(r.Method, r.Path))
		case "DELETE":
			s.Delete(r.Path, shipHandler(r.Method, r.Path))
		}
	}
}
